...
Otherwise, determine the best station via latitude/longitude."
  ...
  (let (station)
	...
	(setq station (cdr (assq 'code (cl-find-if (lambda (s)
     ...
     ((= arg 16)
      (setq station "LGAV"
            ;; (completing-read "Enter METAR station code: "
    ...
    (let ((info (metar-decode (metar-get-record station))))
      (if info
	  (message "%s:    %d minutes ago | %d°%c | %s%d%% humidity | %.1f %S | %d Knots from %d degrees | %d°%c dewpoint."
		   ...
		   (cadr (assoc 'pressure info))
                   ...
                   (caar (cddddr (assoc 'wind info)))
                   (caaddr (assoc 'wind info))
                   (cadr (assoc 'dewpoint info))
       		   (cond
		    ((eq (cdr (assq 'temperature metar-units)) 'degC) ?C)
		    ((eq (cdr (assq 'temperature metar-units)) 'degF) ?F)))        
...
;; 	various functions in calc-units expect already simplified expressions. 
;; 	In practice, it should work around the above bug.
;; 
;; 2016-02-26  Wolfgang Jenkner  <wjenkner@inode.at>
;; 
...
;; 	metar-convert-unit. Pass t as last argument to the underlying calc
;; 	conversion functions so that they return only the number, not the unit,
;; 	thereby fixing the bug in question.
;; 
;; 2014-06-24  Mario Lang	<mlang@delysid.org>
;; 
;; 	Release 0.1.
;; 
;; 2014-06-02  Mario Lang	<mlang@delysid.org>
;; 
;; 	[metar] Unify station lat/lon conversion.
;; 
;; 2014-05-31  Mario Lang	<mlang@delysid.org>
;; 
;; 	[metar] Fix metar-convert-temperature to actually DTRT
;; 
;; 2014-05-30  Mario Lang	<mlang@delysid.org>
;; 
...
;; 
;; 2014-05-29  Mario Lang	<mlang@delysid.org>
;; 
;; 	[metar] Make the length unit configurable (clouds) and simplify pressure
;; 	conversion
;; 
;; 2014-05-27  Mario Lang	<mlang@delysid.org>
;; 
...
;; 
;; 2014-05-26  Mario Lang	<mlang@delysid.org>
;; 
;; 	Fix phenomena decoding
;; 
;; 2014-05-25  Mario Lang	<mlang@delysid.org>
;; 
;; 	Add metar.el.
;; 
